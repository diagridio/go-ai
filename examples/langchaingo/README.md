# LangChainGo example — Control Room Operator

A two-node LangChainGo agent (`diagnose → reboot`) that runs as a durable Dapr
Workflow.
...
diagrid agent create control-room --project $PROJECT --wait
...
```bash
diagrid dev run --project $PROJECT --app-id control-room -e GOWORK=off -- go run .
```
...
```
Agent "control-room" (framework=langchaingo) ran as workflow dapr.langchaingo.control_room.workflow
Diagnosis: <the model's diagnosis of the grid failure>
Operator : <the model's reboot report>
```
...
workflow: each node (`diagnose`, `reboot`) runs as a checkpointed `execute-node`
